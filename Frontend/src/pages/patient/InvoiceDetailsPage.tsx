  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        ...
        setError(err.response?.data?.message || t("paymentError.invoiceFailed", "Failed to load invoice details."));
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4">
        <Card className="p-8 max-w-md w-full text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-slate-800">{t("paymentError.invoiceNotFound", "Invoice not found")}</h2>
          <p className="text-sm text-slate-500">{error || t("paymentError.invoiceFailed", "Could not retrieve invoice details.")}</p>
          <Button onClick={() => navigate("/patient/dashboard")} className="w-full">
            {t("paymentModal.buttons.return", "Back to Dashboard")}
          </Button>
        </Card>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 py-10 px-4 sm:px-6 lg:px-8 print:bg-white print:py-0">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between print:hidden">
          ...
          >
            <ArrowLeft className="w-5 h-5" /> {t("onboarding.back", "Back")}
          </button>
          <div className="flex gap-2">
            <Button variant="secondary" onClick={handlePrint} leftIcon={<Printer className="w-4 h-4" />}>
              {t("paymentModal.buttons.print", "Print Receipt")}
            </Button>
          </div>
        </div>
          ...
          <Card className="p-8 sm:p-12 rounded-3xl border-none shadow-sm space-y-8 bg-white border border-slate-100 print:shadow-none print:border-none">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6 border-b border-slate-100 pb-8">
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-teal-600 flex items-center justify-center text-white font-extrabold text-sm">O</div>
              ...
              <div className="text-left sm:text-right space-y-1">
                <span className="text-xs text-teal-600 uppercase tracking-widest font-extrabold">{t("paymentModal.labels.invoiceTitle", "Official Invoice")}</span>
            ...
            <div className="grid grid-cols-2 gap-6 text-sm">
              <div className="space-y-1">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t("paymentModal.labels.patient", "Patient Details")}</p>
                <p className="font-bold text-slate-800">{invoice.patientName}</p>
                <p className="text-xs text-slate-500">Care Recipient</p>
              </div>
              <div className="space-y-1 text-left sm:text-right">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t("paymentModal.labels.doctor", "Medical Professional")}</p>
